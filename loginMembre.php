...
if(isset($btn_send)){
    ...
    $requete = "SELECT nomUser,motPasse,imageProfil,statut,role FROM equipe";
    $result = $db->query($requete);
    while($donne = $result->fetch()){
        if($passe == $donne["motPasse"] && $donne["statut"] == 1){
            $login = $donne["nomUser"];
            $profil = $donne["imageProfil"];
            $role = $donne["role"];
            $a = 1;
            $_SESSION["nomUser"] = $login;
            $_SESSION["imageProfil"] = $profil;
            $_SESSION["role"] = $role;
    ...
    if($a == 1){
          ...
          if($role == "Social média" || $role ==""){
          echo "<script type='text/javascript'> alert('Connexion valide soyez la bienvenue');</script>";
          echo "<script type='text/javascript'>document.location.replace('principal_1.php');</script>";
          die();
          }
    }
    ...
    if($a == 0){
        ...
        // $affiche = '<div class="erreur"> Erreur nom ou passe</div>';
          echo "<script type='text/javascript'> alert(' Erreur de connexion, contacter l\'admin pour savoir si votre compte a été desactivé');</script>";
...
<p><a href="#">Dashboard</a> <i class="fas fa-caret-right"></i> login membre</p>
</div>
</div>
</div>
</div>
...
<h5 class="modal-title text_white">Authentification Membre</h5>
</div>
<div class="modal-body">
<form method="POST" action="#">
<div class>
<input type="text" name="username" id="username" class="form-control" placeholder="Entrer votre nom ">
</div>
<div class>
<input type="password" name="passe" id="passe" class="form-control" placeholder="Mot de passe">
</div>
<button type="submit" name="send" id="send" class="btn btn-primary btn_1 full_width text-center">Connexion</button>

<div class="text-center">
<a href="#" data-bs-toggle="modal" data-bs-target="#forgot_password" data-bs-dismiss="modal" class="pass_forget_btn">Mot de passe oublié ?</a>
</div>
</form>
</div>
</div>
...
<p>2020 © Dashboard Ingenuis - Designed by Carlos</p>
</div>
</div>
</div>
</div>
