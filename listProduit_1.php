...
$requete = "SELECT * FROM ajoutProduit";
...
if($idProd!=""){
  $req1 = $db->prepare("DELETE FROM ajoutProduit WHERE id = :idProd");
  $req1->bindParam(':idProd', $idProd ,PDO::PARAM_STR);
  $req1->execute();


 echo "<script type='text/javascript'> alert('Produit supprimé avec succes');</script>";
 echo "<script type='text/javascript'>document.location.replace('listProduit.php');</script>";
  }
?>
...
<p>consulter les differents produits du site.</p>
...
while($donne = $resultat->fetch()){
  ...
  $nomConcatene = $donne['imageProd'];
  $tableau = explode(",",$nomConcatene );
...
?>
  ...
  <tbody>
    ...
    <td> <img src=<?php echo "imageProd/".$tableau[0]; ?> alt="rien" width="40" height="40" style="border-radius:80px;"/> </td>
    <td>
		<a href="listProduit.php?idProd=<?php echo $donne['id']?>"> <button class="btn btn-danger">Delete</button> </a>
      </td> 
  </tr>
