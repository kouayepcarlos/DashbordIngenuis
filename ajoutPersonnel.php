...
$passe = isset($_POST["passe"]) ? strip_tags($_POST["passe"]) : "";
$phone = isset($_POST["phone"]) ? strip_tags($_POST["phone"]) : "";
$image = isset($_FILES["image"]['name']) ? $_FILES["image"]['name'] : "";
$role = isset($_POST["role"]) ? strip_tags($_POST["role"]) : "";
$btn_send = isset($_POST["send"]) ? $_POST["send"] : "";
...
if(isset($btn_send)){
 ...
 elseif(empty($role)){
  ...
  $image= $_FILES["image"]["name"];
	move_uploaded_file($_FILES["image"]["tmp_name"], './ImageProfil/'. $image);
  $insert = "INSERT INTO equipe(nomUser,motPasse,phone,imageProfil,role,statut) VALUES(:nomUser, :motPasse,:phone,:imageProfil,:role,:statut)";
  ...
  $result->execute($tabResult);
  
  echo "<script type='text/javascript'> alert('Creation d\'un membre avec succès.');</script>";
  echo "<script type='text/javascript'>document.location.replace('ajoutPersonnel.php');</script>";
...
<h3 class="mb-0">Ajouter Un Membre De Votre Equipe</h3>
<p>Remplisser les informations et choississez son role puis valider.</p>
...
<form method="POST" action="#" class='form' enctype="multipart/form-data">
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
      ...
      <input type="password" class="form-control" name="passe" id="passe" placeholder="Mot de passe" required/>
    </div>
  </div>
  <div class="form-group">
    <label for="phone">Numéro Téléphone</label>
    <input type="text" class="form-control" name="phone" id="phone" placeholder="653286910" required/>
  </div>
  <div class="form-group">
    <label for="image">Image Profile </label>
    <input type="file" class="form-control" name="image" id="image" accept=".jpg, .png, ,svg" required/>
  </div>
  <div class="form-group">
    <label for="role">Choississez le role </label>
    <select id="role" name="role" class="form-select">
        <option selected value="Social média">Social média</option>
        <option value="Conseiller(e) clientèle">Conseiller(e) clientèle</option>
        <option value="Directeur Artistique">Directeur Artitisque</option>
        <option value="Directrice Administrative et Financière">Directrice Administrative et Financière</option>
    </select>
  </div>
 
  <button type="submit" name="send" id="send" class="btn btn-primary">Créer Membre</button>
