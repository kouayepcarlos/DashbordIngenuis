...
if(isset($btn_send)){
    ...
    $requete = "SELECT username,motPasse,profil FROM admin";
    $result = $db->query($requete);
    while($donne = $result->fetch()){
        if($passe == $donne["motPasse"]){
            $login = $donne["username"];
            $profil = $donne["profil"];
            $a = 1;
    ...
    if($a == 1){
          ...
          echo "<script type='text/javascript'>document.location.replace('principal.php?nomUser=$login&profil=$profil');</script>";
          die();
    }
...
<p><a href="#">Dashboard</a> <i class="fas fa-caret-right"></i> login</p>
</div>
</div>
</div>
</div>
...
<h5 class="modal-title text_white">Authentification Admin</h5>
</div>
<div class="modal-body">
<form method="POST" action="#" class='form'>
<div class>
<input type="text" name="username" id="username" class="form-control" placeholder="Entrer votre username " required/>
</div>
<div class>
<input type="password" name="passe" id="passe" class="form-control" placeholder="Password" required/>
</div>
<button type="submit" name="send" id="send" class="btn btn-primary btn_1 full_width text-center">Connexion</button>

<p>Vous etes membre de l'equipe ? <a href="loginMembre.php"> Connexion</a></p>
<div class="text-center">
<a href="#" class="pass_forget_btn">Mot de passe oublié ?</a>
</div>
</form>
</div>
</div>
